...
class Auth extends Controller {
    ...
    function __construct() 
    {
     ...
     /**
     * Changed by Alexis Serneels for optimization
     */
    function index() 
    {
    	redirect('profile');
    ...
    /**
     * ADDED by Alexis Serneels for optimization
     */
    function profile() 
    {
    	if (!$this->ion_auth->logged_in()) {
    		...
    		$this->template->write_view('content', 'auth/profile', $this->data, TRUE);
     ...
     */
    function admin() {
    	// ADMIN SECTION redirect if needed, otherwise display the user list
    	if (!$this->ion_auth->logged_in()) {
	    	//redirect them to the login page
			redirect('auth/login', 'refresh');
    	}
		...
		/**
        * Changed by Alexis Serneels to match the Template Library
        */
		$this->template->write_view('content', 'auth/admin', $this->data, TRUE);
		$this->template->render();
    	}
    }
    ...
    function login() 
    {
        ...
        if ($this->form_validation->run() == true) { //check to see if the user is logging in
        	...
        	if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'), $remember)) { //if the login is successful
	        	...
	        	redirect('auth/profile', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
	        }
             ...
             * Changed by Alexis Serneels to match the Template Library
             */
    		$this->template->write_view('content', 'auth/login', $this->data, TRUE);
			$this->template->render();
		}
    }
	...
	{	    
	    ...
	    if ($this->form_validation->run() == false) { //display the form
             ...
             * Changed by Alexis Serneels to match the Template Library
             */
    		$this->template->write_view('content', 'auth/change_password', $this->data, TRUE);
			$this->template->render();	        
	    }
	    else {
	...
	function forgot_password() 
	{
	    ...
	    if ($this->form_validation->run() == false) {
             ...
             * Changed by Alexis Serneels to match the Template Library
             */
    		$this->template->write_view('content', 'auth/forgot_password', $this->data, TRUE);
			$this->template->render();
	    }
	    else {
	...
	{  
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username,$password,$email,$additional_data)) { //check to see if we are creating the user
             ...
             * Changed by Alexis Serneels to match the Template Library
             */
    		$this->template->write_view('content', 'auth/register', $this->data, TRUE);
			$this->template->render();
		}
    }
