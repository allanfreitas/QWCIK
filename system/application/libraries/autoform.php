 ...
 * html forms Build for the Codeigniter Framework.
 * 
...
class Autoform {
  ...
  private $field_types = array ('timestamp'=>'date','checkbox'=>'checkbox','radio'=>'radio','hidden'=>'hidden','file'=>'file','text'=>'text','blob'=>'textarea','textarea'=>'textarea','select'=>'select','password'=>'password');
   ...
   * @return Mixed
   */
  public function get($field_id, $params=FALSE) {
    $get = array();
    if (is_array($params)) {
  ...
  private function validate($fields) {
    ...
    if (form_error($field->name)) {
      ...
      if (isset($field->label->class) && !preg_match('/error/', $field->label->class)) { 
  ...
  private function input($field) {
    ...
    return form_input($data, $field->value, $this->stringify($extra));
  }
  ...
  private function checked_input($field) {
    ...
    return $method($extra, $field->value, $set_method($field->name, $field->value));
  }
  ...
  private function dropdown($field) {
    ...
    foreach ($field as $key=>$value) {
      if ($key!='name' && $key!='options' && $key != 'value' && $key != 'label') {
        $extra[$key] = $value;
      }
    }
   ...
   * @return String
   */
  public function open($action, $attr=array(), $multipart=FALSE) {
    if ($multipart==TRUE) {
